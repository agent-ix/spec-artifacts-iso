...
def validator():
    ...
    return Draft202012Validator({"$ref": "#/$defs/DocumentReference", "$defs": schema["$defs"]})


def reference():
    return {"name": "functional-coverage", "archetype": "TestMatrix",
            "section": "Functional Requirement Coverage", "column": "Test Cases",
            "pattern": "(TC-[0-9]+)", "targets": ["test-case"]}
...
def test_only_the_additive_property_changes_the_parent_schema():
    ...
    path = pathlib.Path(__file__).resolve().parents[1] / "spec_artifacts_iso/module-manifest.schema.json"
    text = path.read_text()
    addition = '        "status_column": {\n          "type": "string",\n          "pattern": "\\\\S",\n          "description": "quire-rs#409: optional exact status-column override; omission inherits traceability.status.column. The engine requires a declared global status vocabulary."\n        },\n'
    assert text.count(addition) == 1
    assert hashlib.sha256(text.replace(addition, "").encode()).hexdigest() == "52bddd1c14e0df06e95322db45734925990a4472e6b4c980a5c14f00480bb874"

...
| TC-021 | the manifest declares the `edge_types` and `roles` registries; that a consumer accepts the declaration is asserted where a consumer reads it, by loading the module through quire (TC-048) (`test_fr004_ac5_the_manifest_declares_the_vocabulary_registries`) | Unit | P0 | FR-004-AC-5 | ✅ |

...
* **2026-09-22** — PLAT-974: CI off dev mirrors. `.github/workflows/ci.yml`'s
  ...
  `quire` moves from `>=0.33.0` to `^0.47.1` in `pyproject.toml` — the
  `internal-pypi` wheel-publish blocker FR-006 named
  (`agent-ix/quire-rs#392`) is resolved — and the `local-pypi` source
  (`pypi.ix`) is deleted from `pyproject.toml`; `poetry.lock` now names only
  `internal-pypi`. `@agent-ix/semantic-core` already resolved from GitHub
  Packages in the committed `package-lock.json`; FR-005 and NFR-001 are
  corrected to say so instead of naming the dev-only `npm.ix` mirror as
  today's route. Re-probed against the installed 0.47.1 wheel: the four
  malformed-`semantic`-block mutations FR-006 Behavior describes for "quire
  0.46.0" still hold (three empty the registry, the ambiguous `data_schema`
  drops one archetype); FR-006-AC-4 stays retired (PLAT-902) and only the
  digest-binding half (`agent-ix/quire-rs#388`) remains blocked. All four
  local gates pass with no drift; no xfail flipped to XPASS.
...
* **2026-09-20** — **PLAT-902: this package stops redistributing filament-core-service's FR-035 module-manifest schema.** Deleted `spec_artifacts_iso/module-manifest.schema.json`, the public `module_manifest_schema()` / `MODULE_MANIFEST_SCHEMA_PATH` API, and the schema's entries in the wheel `include`, the npm `files` list and the npm staging payload. **FR-001 CR-002 is retired**, and its record now says so where it is read. CR-002's reasoning was that "a copy per repo is a copy that drifts", and its cure was one copy shipped from here — but redistributing a copy is copying it. What shipped was this repository's fork of the schema, carrying the original's `$id`, so four sibling repositories that gated their manifests on it were comparing a manifest to bytes maintained here, not to filament-core-service's contract. **The trap it set is visible in what had to be written around it**: FR-006-AC-4 required the copy to stay byte-identical to the private revision `a77f31e`, so FR-006's Inputs had to carry, as permanent text, two defects in that revision — an ECMA lookahead the Rust `jsonschema` crate cannot compile (`agent-ix/filament-core-service#24`) and a `semantic.exports` description contradicting this module's own exports (`agent-ix/quoin#336`) — because the criterion forbade fixing them. Downstream, `spec-artifacts-process` had to delete the `semantic` block and the `traceability.trace_targets[].required` key from its manifest before its copy would accept it, and validate the deleted block against a *second* copy.
  ...
  **[RAN] FR-006-AC-4 is retired rather than restated over quire's loader, and the measurement is why.** At this module's declared engine floor — quire 0.33.0, the published floor of the internal index — the loader ignores the `semantic` block entirely: an undeclared key, `package: ix://agent-ix/x` and `targets: [go]` each still load all eleven archetypes, and an ambiguous `data_schema` does too. Against quire 0.46.0 all four are refused (the first three empty the registry; the ambiguous reference drops that one artifact type). 0.46.0 is in no published wheel (`agent-ix/quire-rs#392`). A loader-based criterion at the declared floor would therefore have passed by not running — the exact failure CR-002 was filed to close — so it is recorded in FR-006 Behavior with the numbers and becomes observable when the wheel is published.

* **2026-09-04** — **#34: the semantic layer.** The module now declares one TypeSpec model per ISO artifact type (FR-005), emits their JSON Schema 2020-12 projection with the official `@typespec/json-schema` emitter, binds each archetype to the shipped bytes by digest through the quoin FR-070 `semantic` block (FR-006), and publishes the Markdown-to-record mapping and ten golden records (FR-007), reproducibly and offline (NFR-001). `scripts/corpus_census.py` became a committed, tested deliverable and re-measuring corrected six figures FR-005 quoted — most importantly "30 distinct constraint categories", which was a `most_common(30)` display limit rather than a measurement; the true count is 334. Two upstream gaps were filed rather than worked around: agent-ix/quoin#336 (`semantic.exports` resolves against `object_types` only, so artifact-type exports are refused) and agent-ix/quire-rs#393 (FR-069's prose says "object type" while the loader admits every archetype). The one criterion needing the unpublished FR-069 wheel is a strict expected failure naming agent-ix/quire-rs#388, never a skip.
...
* **2026-08-21** — FR-001 CR-011: `source_exclude` items gain **value constraints** — CR-010's "`tests/**` MUST NEVER appear here" moves from prose into the schema (agent-ix/spec-artifacts-iso#29). Rejected: bare `**`, patterns opening with a wildcard (`*`/`?` — globset's `literal_separator=false` makes `*/fixtures/**` match at any depth), and any form naming the `tests` tree with no literal anchor after it — every later segment empty or wildcard-carrying (`tests`, `tests/`, `tests/**`, `tests/**/*.py`, `tests/x*`, `tests/f*/**`) — the semantics the spec-artifacts-process contract test pins, while a literal segment re-anchors at any depth (`tests/**/fixtures/**` stays legal; regex fixed to the segment rule in the SR-002 review pass). Deliberately narrower than a blanket `tests/` prefix ban: the anchored `tests/fixtures/**` form CR-010 recommends — and spec-artifacts-process ships — stays legal. Before this, `["tests/**"]` and `["**"]` validated cleanly and silently subtracted the evidence tree from the symbol walk. Also splits the shared test number found in the PR #28 retro review (FND-004): `test_tc_schema_027_source_exclude_is_not_exclude` renumbers to `028` with its own row TC-036. TC-037, TC-038.

* **2026-08-20** — FR-001 CR-010: the fixture gains `traceability.source_exclude[]` (quire-rs FR-050-AC-22 / CR-085) — path globs under the **code** root naming fixture trees that hold no traceable source. **A separate key from `exclude`, deliberately**: `exclude` scopes documents and has never been applied to a source file, while `spec-artifacts-process` FR-004-AC-9 *requires* trace targets to exclude `tests/**` and 194 of quire-rs's own ~458 trace markers live under `tests/`. One key meaning both would delete the evidence tree and read as an ecosystem-wide regression, so `tests/**` must never appear here — declarations anchor at `tests/fixtures/**`. Filed for the reason CR-005/007/008/009 were: without the key, `additionalProperties: false` rejects the manifest before the engine sees it, so no module could declare what quire-rs v0.41.0 already reads. TC-035.

* **2026-08-19** — FR-001 CR-009: the fixture gains `trace_tags.implements[]`, the same `TraceMarkerForm` shape as `markers` (quire-rs FR-062). **A separate list, deliberately**: `markers` mint `verifies`, which may back an acceptance criterion; these mint `implements`, which never may. quire-rs CR-061 stopped `verifies` binding production symbols precisely because a doc comment citing a criterion would otherwise count as evidence for it, and a shared list with a discriminator would put one typo between scope and evidence. `TraceMarkerForm` stays `additionalProperties: false` so a module cannot invent that discriminator — TC-033 asserts the rejection rather than trusting it. Without this key a module declaring the forms fails load outright, so quire-rs FR-062 would mint nothing however the engine behaved. TC-032..026.

* **2026-08-19** — FR-001 CR-008: the fixture gains `ObligationSource.combinatorial` and a `CombinatorialColumns` definition (quire-rs FR-061) — read a source's table as a **configuration space** and mint ONE obligation for the whole table rather than one per row. A new source *kind*, not a new mechanism: hash, suspect link and `parameters` are FR-053's; what differs is arity, because a t-way obligation is a statement about the interaction of every row. `strength` is rejected at `0` and unknown keys are rejected outright — a plural typo like `dimensions_column` reads correctly and would mint nothing. **Filed because the engine half shipped in Wave D with no module able to declare it**: `additionalProperties: false` rejected the key, so quire-rs FR-061 and quoin FR-035 could never fire and zero combinatorial obligations existed anywhere in the ecosystem. TC-029..023.

* **2026-08-18** — FR-001 CR-007: the fixture gains `traceability.vocabulary_coverage[]` and a `VocabularyCoverage` definition (quire-rs FR-059) — which declared vocabulary values no document claims. **The definition has no `values` key on purpose**: the vocabulary is read from the projected archetype's own frontmatter-schema `enum`, and `additionalProperties: false` makes a module writing one an error rather than a silently ignored line. The 25010 list already lives in this repo's `schemas/nfr-frontmatter.schema.json` with twelve values; quire-rs#162 was filed against a scope proposing a hardcoded nine-item copy. TC-026..020.

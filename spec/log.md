...
* **2026-08-21** — FR-001 CR-011: `source_exclude` items gain **value constraints** — CR-010's "`tests/**` MUST NEVER appear here" moves from prose into the schema (agent-ix/spec-artifacts-iso#29). Rejected: bare `**`, patterns opening with a wildcard (`*`/`?` — globset's `literal_separator=false` makes `*/fixtures/**` match at any depth), and a wildcard directly under `tests/` (`tests/**`). Deliberately narrower than a blanket `tests/` prefix ban: the anchored `tests/fixtures/**` form CR-010 recommends — and spec-artifacts-process ships — stays legal. Before this, `["tests/**"]` and `["**"]` validated cleanly and silently subtracted the evidence tree from the symbol walk. Also splits the shared test number found in the PR #28 retro review (FND-004): `test_tc_schema_027_source_exclude_is_not_exclude` renumbers to `028` with its own row TC-036. TC-037, TC-038.

...
> **CR-013 (explicit reference status column — 2026-09-06, quire-rs#409):**
> `DocumentReference` MAY declare `status_column` as a nonblank string. Omission
> preserves the existing global `traceability.status.column`; the schema adds
> no default, required field, header alias or fallback. The engine owns the
> cross-field rule that an override requires a declared global status vocabulary.
> Blank and non-string overrides SHALL fail this schema. TC-064/TC-065 qualify
> the additive field and the unchanged parent schema; no semantic model schema
> or authoring skeleton is regenerated by this amendment.
